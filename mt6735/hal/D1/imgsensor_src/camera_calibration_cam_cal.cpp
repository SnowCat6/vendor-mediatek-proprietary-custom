...
#endif
#define CAM_CAL_LOG_IF(cond, ...)      do { if ( (1) ) { CAM_CAL_LOG(__VA_ARGS__); } }while(0)
...
UINT32 DoCamCalPartNumber(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	/*LukeHu+-150908=For code defects*/
    /*memcpy((char*)&pCamCalData->PartNumber[0],ucModuleNumber,sizeof(CAM_CAL_PART_NUMBERS_COUNT_BYTE));*/
	memcpy((char*)&pCamCalData->PartNumber[0], ucModuleNumber, CAM_CAL_PART_NUMBERS_COUNT_BYTE);
    #ifdef DEBUG_CALIBRATION_LOAD
...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	UINT8 tempGain, CalR = 0, CalGr = 0, CalGb = 0,
		CalG = 0, CalB = 0, FacR = 0, FacGr = 0,
		FacGb = 0, FacG =0, FacB = 0;
...
//            ShowCmdErrorLog(pCamCalData->Command);
        ...
        CAM_CAL_LOG_IF(dumpEnable,"u4Offset=%d u4Length=%d pu1Params= 0x%x ", cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);
        ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
        if(!ioctlerr)
        {
          err = CAM_CAL_ERR_NO_SHADING;
        }
        CAM_CAL_LOG_IF(dumpEnable,"lsc table_size %d\n",table_size);
...
UINT32 DoCamCalN3dLsc(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    else
    {
        ...
        else
        {
			...
			/*LukeHu+-150908=For code defects*/
            cam_calCfg.u4Offset = 0xFFFF;/*((start_addr>>16)|0xFFFF);*/
            cam_calCfg.u4Length = BlockSize; //sizeof(ucModuleNumber)
...
UINT32 DoCamCalLayoutCheck(void)
{
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0]) == 0))
    {
		...
		CAM_CAL_LOG_IF(dumpEnable,"open eeporm device=%s lCamcamFID= 0x%x", cBuf, lCamcamFID);
        if(lCamcamFID == -1)
        {
    ...
    {
        CAM_CAL_LOG_IF(dumpEnable,"----gIsInitedCamCal = 0x%x!----\n",gIsInitedCamCal);
        CAM_CAL_LOG_IF(dumpEnable&&gIsInitedCamCal !=0,"----NO CAM_CAL name = %s!----\n",cBuf);
		CAM_CAL_LOG_IF(dumpEnable&&gIsInitedCamCal !=0,"----NO CCAM_CALInit = %d!----\n",CAM_CALInit());
    }
    return  result;
}
