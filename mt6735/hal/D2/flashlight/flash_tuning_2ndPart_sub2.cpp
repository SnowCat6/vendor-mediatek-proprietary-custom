#define LOG_TAG "flash_tuning_2ndpart_sub2.cpp"
...
static void copyTuningPara(FLASH_TUNING_PARA* p, NVRAM_FLASH_TUNING_PARA* nv_p)
{
    ...
    ALOGD("copyTuningPara sub 2ndpart yTarget=%d", p->yTarget);
}
...
FLASH_PROJECT_PARA& cust_getFlashProjectPara_sub2 (int aeScene, int isForceFlash, NVRAM_CAMERA_STROBE_STRUCT* nvrame)
{
	...
	if(nvrame!=0)
	{
		...
		ALOGD("cust_getFlashProjectPara ind=%d", ind);
...
void cust_getFlashHalTorchDuty_sub2(int* duty, int* dutyLt)
{
}
