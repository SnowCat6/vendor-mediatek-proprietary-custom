...
#define EEPROM_VER "0x8300"   //83 : 6583, 00 : draft version 120920

#ifdef EEPROM_SHOW_LOG
//#define CAMEEPROM_LOG(fmt, arg...)    LOGD(fmt, ##arg)
#define CAMEEPROM_LOG(fmt, arg...)    ALOGD(EEPROM_CAL_LOG_TAG EEPROM_VER " " fmt, ##arg)
#define CAMEEPROM_ERR(fmt, arg...)    ALOGE(EEPROM_CAL_LOG_TAG EEPROM_VER "Err: %5d: " fmt, __LINE__, ##arg)
#else
...
UINT32 DoPregainLoad(INT32 epprom_fd, UINT32 start_addr, UINT32* pGetSensorCalData)
{
    ...
    CAMEEPROM_LOG("DoPregainLoad(0x%x)----\n",(uintptr_t)pGetSensorCalData);
